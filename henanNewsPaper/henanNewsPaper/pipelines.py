...
class HenannewspaperPipeline(object):
    def __init__(self):
        self.wb = Workbook()
        self.ws = self.wb.active
        self.ws.append(['时间', '标题', '内容', '来源', '链接'])

    def process_item(self, item, spider):
        line = [item['publishtime'], item['title'], item['content'], item['fromwhere'], item['url']]
        self.ws.append(line)
        self.wb.save(r"河南日报.xlsx")
        return item
