...
class XjbPipeline(object):
    def __init__(self):
        self.wb = Workbook()
        self.ws = self.wb.active
        self.ws.append(['时间', '出处', '标题', '内容',  '链接', '编辑', '作者'])

    def process_item(self, item, spider):
        line = [item['time'], item['fromWhere'], item['title'], item['content'], item['url'], item['editor'], item['author']]
        self.ws.append(line)
        self.wb.save(r"C:\Users\asus\Desktop\pydata\京报网.xlsx")
        return item
