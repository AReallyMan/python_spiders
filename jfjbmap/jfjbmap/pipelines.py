...
class JfjbmapPipeline(object):
    def __init__(self):
        self.wb = Workbook()
        self.ws = self.wb.active
        self.ws.append(['时间', '标题', '内容', '链接'])

    def process_item(self, item, spider):
        line = [item['publishtime'], item['title'], item['content'], item['url']]
        self.ws.append(line)
        self.wb.save(r"C:\Users\asus\Desktop\pydata\解放军报.xlsx")
        return item
